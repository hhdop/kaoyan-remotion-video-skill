...
test('inspects an SRT without mutating a project', () => {
  const report = inspectSrt(fixture('knowledge.srt'));

  assert.equal(report.valid, true);
  assert.equal(report.cueCount, 5);
  assert.equal(report.startSeconds, 0);
  assert.equal(report.endSeconds, 10);
});
...
test('builds 408-oriented semantic stages from transcript cues', () => {
  ...
  assert.equal(model.durationInFrames, 910);
...
test('writes generatedContent.ts that Remotion template can import', () => {
  ...
  assert.match(generated, /export const generatedVideo: GeneratedVideo/);
...
test('builds a planning timeline without inventing a course change', () => {
  const cues = parseSrt(fs.readFileSync(fixture('planning.srt')));
  ...
  assert.equal(model.profile, 'planning');
  assert.deepEqual(
    model.stages.map((stage) => stage.label),
    ['结论', '数学', '408', '英语', '政治', '总结'],
  );
  assert.ok(model.stages.every((stage) => !stage.main.includes('原专业课')));
  assert.equal(model.durationSeconds, 12);
  assert.equal(model.durationInFrames, 360);
});

test('selects news and knowledge profiles from explicit transcript evidence', () => {
  const news = buildContentModel(parseSrt(fs.readFileSync(fixture('news.srt'))), {
    profile: 'auto',
    audioDurationSeconds: 10,
  });
  const knowledge = buildContentModel(parseSrt(fs.readFileSync(fixture('knowledge.srt'))), {
    profile: 'auto',
    audioDurationSeconds: 10,
  });

  assert.equal(news.profile, 'news');
  assert.equal(knowledge.profile, 'knowledge');
  assert.ok(news.stages.some((stage) => stage.main.includes('408')));
  assert.ok(knowledge.stages.every((stage) => !/学校|公告|专业课/.test(stage.main)));
});

test('normalizes stage coverage so subtitle gaps hold the preceding stage', () => {
  const cues = [
    {start: 0, end: 1, text: '先提出一个问题'},
    {start: 1.4, end: 2.5, text: '再解释核心概念'},
    {start: 2.5, end: 3, text: '最后总结方法'},
  ];
  const model = buildContentModel(cues, {
    profile: 'knowledge',
  ...
  assert.equal(model.stages[0].start, 0);
  for (let index = 1; index < model.stages.length; index += 1) {
    assert.equal(model.stages[index - 1].end, model.stages[index].start);
  }
  assert.equal(model.stages.at(-1).end, 3);
  assert.ok(
    model.stages.every((stage) =>
      stage.points.every((point) => point.cueStart >= stage.start && point.cueEnd <= stage.end),
    ),
  );
});
