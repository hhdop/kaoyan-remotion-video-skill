...
The reusable template enforces a relative-luminance ceiling of `0.82` for major background, card, paper, and surface colors. Accent chips may be brighter because they are not major surfaces.
...
- A chapter handoff must not create a content vacuum or transparent Chinese text double-exposure. Keep the outgoing stage visible through the cue frame, then use complementary clipping or another non-overlapping handoff so old and new coverage totals 100% while the transition runs.
...
For other profiles, preserve the same hierarchy without copying those exact props:

- Planning: a priority rail or plan row commits first; a meter follows; the loop or summary settles last.
- Knowledge: the question establishes the frame; concept nodes appear only as spoken; connectors appear only when two revealed nodes genuinely relate.
- News: source/fact structure leads; an explicit before/after comparison is allowed only when the transcript provides both sides.
...
- No major chapter boundary contains an empty content panel, accidental full-paper flash, or overlapping old/new copy.
